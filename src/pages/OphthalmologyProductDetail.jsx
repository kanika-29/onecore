  ...
  if (parts.length >= 3) {
    ...
    return [
      ...
      },
      {
        ...
        desc: "Supports intended ocular physiological balance and target clinical relief."
      }
    ];
  } else {
    return [
      ...
      },
      {
        num: "02",
        title: "Target Pathway",
        desc: "Operates through specific ocular receptors and tissue-level biochemical pathways."
      },
      {
        num: "03",
...
function parsePrecautionsAccordions(precautionsArray) {
  ...
  return precautionsArray.map((text, idx) => {
    ...
    if (lower.includes("dropper") || lower.includes("tip") || lower.includes("discard") || lower.includes("contact lens")) {
      title = "Aseptic Technique & Contact Lens Precautions";
    } else if (lower.includes("steroid") || lower.includes("intraocular pressure") || lower.includes("cataract") || lower.includes("glaucoma")) {
      title = "Intraocular Pressure & Steroid Monitoring";
    } else if (lower.includes("timolol") || lower.includes("asthma") || lower.includes("bradycardia") || lower.includes("heart block")) {
      title = "Cardiorespiratory & Systemic Beta-Blocker Warnings";
    } else if (lower.includes("cornea") || lower.includes("healing") || lower.includes("bleeding")) {
      title = "Corneal Safety & Bleeding Considerations";
    } else if (lower.includes("fungal") || lower.includes("keratitis") || lower.includes("vision-threatening")) {
      title = "Microbial & Fungal Keratitis Urgency";
    } else if (lower.includes("angle-closure") || lower.includes("dilation") || lower.includes("phenylephrine") || lower.includes("blurred")) {
      title = "Angle-Closure & Visual Accommodation Warning";
    } else if (lower.includes("brimonidine") || lower.includes("sulfonamide") || lower.includes("carbonic-anhydrase")) {
      title = "Adrenergic & Carbonic Anhydrase Considerations";
    }
...
export default function OphthalmologyProductDetail() {
  ...
  useEffect(() => {
    if (product) {
      document.title = `${product.name} — Eyerix | Onecore Pharma`;
    } else {
    ...
    return (
      <div className="w-full bg-[#f7f5f1] text-[#232126] min-h-screen">
        <div className="border-b border-[#d9d4cf] py-3 px-6 text-xs text-[#777078]">
          <Link to="/areas-of-care/eyerix" className="hover:text-[#232126]">Areas of Care › Ophthalmology</Link>
        </div>
        <div className="max-w-4xl mx-auto px-4 py-20 text-center space-y-6">
          <h1 className="font-serif text-4xl text-[#232126]">Product Not Found</h1>
          <p className="text-base text-[#625d64]">
            The requested Ophthalmology product "<span className="font-mono">{productSlug}</span>" could not be found.
          </p>
          <div className="pt-4">
            <Link
              to="/areas-of-care/eyerix"
            ...
            >
              ← Back to Ophthalmology Products
            </Link>
          </div>
        </div>
      </div>
    ...
    <div className="w-full bg-white text-[#232126] font-sans antialiased">
      ...
      <div className="w-full border-b border-[#d9d4cf] bg-white py-3 px-6 sm:px-12 flex flex-col sm:flex-row justify-between items-start sm:items-center text-xs text-[#777078] gap-2">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <span>›</span>
          <Link to="/areas-of-care/eyerix" className="hover:text-[#232126] transition-colors">Ophthalmology</Link>
          <span>›</span>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          <div className="lg:col-span-9 space-y-8">
            <h2 className="font-serif text-4xl sm:text-5xl font-normal leading-tight max-w-3xl">
              How the formula operates in the eye.
            </h2>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          <div className="lg:col-span-9 space-y-8">
            ...
            <div className="mt-8 bg-[#efe9f2] border-l-4 border-[#5b2a70] p-5 text-sm leading-relaxed text-[#514b53] max-w-3xl">
              <strong className="text-[#5b2a70] font-semibold">Important:</strong> Use only as directed by an ophthalmologist or healthcare professional and according to the locally approved product label. Follow physician instructions.
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
            ...
            <div className="lg:col-span-9 space-y-6">
              ...
              <p className="text-base text-[#625d64] max-w-3xl mb-8">
                Review critical safety instructions, ophthalmic considerations, and contraindications.
              </p>
