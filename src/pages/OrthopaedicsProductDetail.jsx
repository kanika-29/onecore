  ...
  if (parts.length >= 3) {
    ...
    return [
      ...
      },
      {
        ...
        desc: "Supports intended physiological balance and musculoskeletal care."
      }
    ];
  } else {
    return [
      ...
      },
      {
        num: "02",
        title: "Target Pathway",
        desc: "Operates through specific cellular receptors and connective tissue pathways."
      },
      {
        num: "03",
...
function parsePrecautionsAccordions(precautionsArray) {
  ...
  return precautionsArray.map((text, idx) => {
    ...
    } else if (lower.includes("diacerein") || lower.includes("diarrhoea") || lower.includes("liver")) {
      title = "Gastrointestinal & Hepatic Monitoring";
    } else if (lower.includes("nsaid") || lower.includes("paracetamol") || lower.includes("bleeding") || lower.includes("ulcer")) {
      title = "Analgesic, Bleeding & Gastrointestinal Risk";
    } else if (lower.includes("thiocolchicoside") || lower.includes("pregnancy") || lower.includes("seizure")) {
      title = "Pregnancy & Neurological Restrictions";
    } else if (lower.includes("surgery") || lower.includes("bromelain") || lower.includes("anticoagulant")) {
      title = "Surgical & Haemostatic Precautions";
    } else if (lower.includes("calcium") || lower.includes("vitamin")) {
...
export default function OrthopaedicsProductDetail() {
  ...
  useEffect(() => {
    if (product) {
      document.title = `${product.name} — Ortheon | Onecore Pharma`;
    } else {
    ...
    return (
      <div className="w-full bg-[#f7f5f1] text-[#232126] min-h-screen">
        <div className="border-b border-[#d9d4cf] py-3 px-6 text-xs text-[#777078]">
          <Link to="/areas-of-care/ortheon" className="hover:text-[#232126]">Areas of Care › Orthopaedics</Link>
        </div>
        <div className="max-w-4xl mx-auto px-4 py-20 text-center space-y-6">
          <h1 className="font-serif text-4xl text-[#232126]">Product Not Found</h1>
          <p className="text-base text-[#625d64]">
            The requested Orthopaedics product "<span className="font-mono">{productSlug}</span>" could not be found.
          </p>
          <div className="pt-4">
            <Link
              to="/areas-of-care/ortheon"
  ...
  const isOneFlexo = product.slug === 'oneflexo';
    ...
    <div className="w-full bg-white text-[#232126] font-sans antialiased">
      ...
      <div className="w-full border-b border-[#d9d4cf] bg-white py-3 px-6 sm:px-12 flex flex-col sm:flex-row justify-between items-start sm:items-center text-xs text-[#777078] gap-2">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <span>›</span>
          <Link to="/areas-of-care/ortheon" className="hover:text-[#232126] transition-colors">Orthopaedics</Link>
          <span>›</span>
      ...
      <div className="w-full bg-[#f7f5f1] grid grid-cols-1 lg:grid-cols-12 min-h-[640px]">
        ...
        <div className="lg:col-span-5 bg-gradient-to-br from-[#e9e5df] to-[#f1eee9] p-8 sm:p-14 lg:p-16 flex flex-col items-center justify-center relative min-h-[420px]">
          {isOneFlexo ? (
            ...
            <div className="w-[220px] sm:w-[260px] bg-white border border-[#d4cfc9] rounded-[16px] shadow-lg p-6 flex flex-col items-center relative min-h-[340px]">
              ...
              <div className="w-full pt-3 border-t border-[#eeeae6] text-center">
                <span className="text-[9px] font-mono uppercase tracking-widest text-[#887e8c]">ORTHEON · ORTHOPAEDICS</span>
              </div>
            </div>
          )}
          <div className="absolute bottom-4 left-6 bg-white/90 backdrop-blur-xs px-3 py-1.5 rounded-xs text-[11px] text-[#736d74] border border-[#e5e1dc]">
            Orthopaedic Healthcare Formulation
          </div>
        </div>
