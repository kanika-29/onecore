...
import { getEntProductBySlug } from '../data/entProducts';
  ...
  if (parts.length >= 3) {
    ...
    return [
      ...
      },
      {
        ...
        desc: "Reduces mucosal inflammation and provides symptomatic relief in upper respiratory structures."
      }
    ];
  } else {
    return [
      ...
      },
      {
        num: "02",
        title: "Target Pathway",
        desc: "Acts on specific receptors, mucosal tissues and inflammatory pathways."
      },
      {
        num: "03",
        title: "Therapeutic Effect",
        desc: "Helps achieve the intended clinical response under medical guidance."
      }
    ];
  }
}
...
function parseUsedForItems(usedForText) {
  ...
  const items = usedForText
    ...
    .split(/(?:;|,|\band\b|\bor\b|—)/)
    .map(i => i.trim())
    .filter(i => i.length > 3 && !i.match(/^(?:for|or|and|not for viral|not as a stand-alone|not appropriate for)$/i));
...
function parseDirectionSteps(directionText) {
  ...
  return sentences.map((sentence, idx) => {
    ...
    if (idx === 0) title = "Dosage & Initial Administration";
    else if (idx === 1) title = "Technique & Timing";
    else if (idx === 2) title = "Dose Adjustment & Maintenance";
...
function parsePrecautionsAccordions(precautionsArray) {
  ...
  return precautionsArray.map((text, idx) => {
    ...
    if (lower.includes("steroid") || lower.includes("cyp3a4") || lower.includes("growth") || lower.includes("adrenal")) {
      title = "Corticosteroid & Systemic Exposure Considerations";
    } else if (lower.includes("infection") || lower.includes("glucose") || lower.includes("blood pressure") || lower.includes("tapering")) {
      title = "Systemic Steroid Screening & Tapering Protocol";
    } else if (lower.includes("penicillin") || lower.includes("antibiotic") || lower.includes("difficile") || lower.includes("diarrhoea")) {
      title = "Hypersensitivity & Antibiotic-Associated Risks";
    } else if (lower.includes("montelukast") || lower.includes("neuropsychiatric") || lower.includes("mood") || lower.includes("sleep")) {
      title = "Neuropsychiatric Monitoring & Allergen Management";
    } else if (lower.includes("inhalation only") || lower.includes("steam") || lower.includes("vapour") || lower.includes("swallow")) {
      title = "Inhalation Route Safety & Thermal Precautions";
    } else if (lower.includes("berberine") || lower.includes("pregnancy") || lower.includes("anticoagulant") || lower.includes("bromelain")) {
      title = "Botanical Interactions & Pregnancy Precautions";
    } else if (lower.includes("antioxidant") || lower.includes("cancer") || lower.includes("oncologist")) {
      title = "Nutraceutical Usage & Adjunct Therapy Guidance";
    }
...
export default function EntProductDetail() {
  ...
  useEffect(() => {
    if (product) {
      document.title = `${product.name} — OTIRA | Onecore Pharma`;
    } else {
    ...
    return (
      <div className="w-full bg-[#f7f5f1] text-[#232126] min-h-screen">
        <div className="border-b border-[#d9d4cf] py-3 px-6 text-xs text-[#777078]">
          <Link to="/areas-of-care/otira" className="hover:text-[#232126]">Areas of Care › ENT</Link>
        </div>
        <div className="max-w-4xl mx-auto px-4 py-20 text-center space-y-6">
          <h1 className="font-serif text-4xl text-[#232126]">Product Not Found</h1>
          <p className="text-base text-[#625d64]">
            The requested ENT product "<span className="font-mono">{productSlug}</span>" could not be found.
          </p>
          <div className="pt-4">
            <Link
              to="/areas-of-care/otira"
    ...
    <div className="w-full bg-white text-[#232126] font-sans antialiased">
      ...
      <div className="w-full border-b border-[#d9d4cf] bg-white py-3 px-6 sm:px-12 flex flex-col sm:flex-row justify-between items-start sm:items-center text-xs text-[#777078] gap-2">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <span>›</span>
          <Link to="/areas-of-care/otira" className="hover:text-[#232126] transition-colors">ENT</Link>
          <span>›</span>
        ...
        <div className="lg:col-span-5 bg-gradient-to-br from-[#e9e5df] to-[#f1eee9] p-8 sm:p-14 lg:p-16 flex flex-col items-center justify-center relative min-h-[420px]">
          <div className="w-[220px] sm:w-[260px] bg-white border border-[#d4cfc9] rounded-[16px] shadow-lg p-6 flex flex-col items-center relative min-h-[340px]">
            ...
            <div className="w-full pt-3 border-t border-[#eeeae6] text-center">
              <span className="text-[9px] font-mono uppercase tracking-widest text-[#887e8c]">OTIRA · ENT</span>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          <div className="lg:col-span-9 space-y-8">
            <h2 className="font-serif text-4xl sm:text-5xl font-normal leading-tight max-w-3xl">
              How the formulation acts in targeted upper-airway pathways.
            </h2>
      ...
      <section id="used" className="bg-[#ebe7e1] text-[#232126] py-20 px-6 sm:px-16">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          <div className="lg:col-span-9 space-y-8">
            <h2 className="font-serif text-4xl sm:text-5xl font-normal leading-tight max-w-3xl">
              Targeted indications and clinical situations.
            </h2>
      ...
      <section id="directions" className="bg-white text-[#232126] py-20 px-6 sm:px-16">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          <div className="lg:col-span-9 space-y-8">
            <h2 className="font-serif text-4xl sm:text-5xl font-normal leading-tight max-w-3xl">
              Administration, dosage, and usage instructions.
            </h2>
            ...
            <div className="mt-8 bg-[#efe9f2] border-l-4 border-[#5b2a70] p-5 text-sm leading-relaxed text-[#514b53] max-w-3xl">
              <strong className="text-[#5b2a70] font-semibold">Important:</strong> Use strictly as prescribed by a qualified healthcare professional and in accordance with local regulatory labelling.
            </div>
          </div>
        </div>
      </section>
