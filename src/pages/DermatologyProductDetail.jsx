...
import { getDermatologyProductBySlug } from '../data/dermatologyProducts';
  ...
  if (parts.length >= 3) {
    ...
    return [
      ...
      },
      {
        ...
        desc: "Supports intended epidermal barrier recovery and dermatological care."
      }
    ];
  } else {
    return [
      ...
      },
      {
        num: "02",
        title: "Target Pathway",
        desc: "Operates through specific cutaneous receptors, enzymes and barrier restoration pathways."
      },
      {
        num: "03",
...
function parseUsedForItems(usedForText) {
  ...
  const items = usedForText
    .replace(/^Used (?:for|when|in|every day on|only for) /i, '')
    .split(/(?:,|\band\b|\bor\b|—)/)
    .map(i => i.trim())
    .filter(i => i.length > 3 && !i.match(/^(?:for|or|and|not as a treatment for|not as a substitute for|not for undiagnosed)$/i));
...
function parseDirectionSteps(directionText) {
  ...
  return sentences.map((sentence, idx) => {
    let title = "Application Instruction";
    if (idx === 0) title = "Dosing & Application";
    else if (idx === 1) title = "Application Advice";
    else if (idx === 2) title = "Duration & Photoprotection";
    else title = "Dermatological Supervision";
...
function parsePrecautionsAccordions(precautionsArray) {
  ...
  return precautionsArray.map((text, idx) => {
    ...
    if (lower.includes("steroid") || lower.includes("atrophy") || lower.includes("striae") || lower.includes("telangiectasia")) {
      title = "Corticosteroid Potency & Cutaneous Atrophy Warning";
    } else if (lower.includes("external use") || lower.includes("eyes") || lower.includes("mucosa")) {
      title = "External Application & Ocular Avoidance";
    } else if (lower.includes("sunscreen") || lower.includes("spf") || lower.includes("uv") || lower.includes("photoprotection")) {
      title = "Photoprotection & UV Reapplication Guidelines";
    } else if (lower.includes("resistance") || lower.includes("antibiotic") || lower.includes("mupirocin") || lower.includes("fusidic")) {
      title = "Antimicrobial Stewardship & Resistance Prevention";
    } else if (lower.includes("paraffin") || lower.includes("flammab") || lower.includes("flame") || lower.includes("fire")) {
      title = "Emollient Flammability & Fire Safety";
    } else if (lower.includes("tretinoin") || lower.includes("pregnancy") || lower.includes("retinoid")) {
      title = "Pregnancy & Teratogenic Contraindications";
    } else if (lower.includes("dryness") || lower.includes("peeling") || lower.includes("irritation")) {
      title = "Local Irritation & Introduction Protocol";
    }
...
export default function DermatologyProductDetail() {
  ...
  useEffect(() => {
    if (product) {
      document.title = `${product.name} — Vellis | Onecore Pharma`;
    } else {
    ...
    return (
      <div className="w-full bg-[#f7f5f1] text-[#232126] min-h-screen">
        <div className="border-b border-[#d9d4cf] py-3 px-6 text-xs text-[#777078]">
          <Link to="/areas-of-care/vellis" className="hover:text-[#232126]">Areas of Care › Dermatology</Link>
        </div>
        <div className="max-w-4xl mx-auto px-4 py-20 text-center space-y-6">
          <h1 className="font-serif text-4xl text-[#232126]">Product Not Found</h1>
          <p className="text-base text-[#625d64]">
            The requested Dermatology product "<span className="font-mono">{productSlug}</span>" could not be found.
          </p>
          <div className="pt-4">
            <Link
              to="/areas-of-care/vellis"
    ...
    <div className="w-full bg-white text-[#232126] font-sans antialiased">
      ...
      <div className="w-full border-b border-[#d9d4cf] bg-white py-3 px-6 sm:px-12 flex flex-col sm:flex-row justify-between items-start sm:items-center text-xs text-[#777078] gap-2">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <span>›</span>
          <Link to="/areas-of-care/vellis" className="hover:text-[#232126] transition-colors">Dermatology</Link>
          <span>›</span>
        ...
        <div className="lg:col-span-5 bg-gradient-to-br from-[#e9e5df] to-[#f1eee9] p-8 sm:p-14 lg:p-16 flex flex-col items-center justify-center relative min-h-[420px]">
          <div className="w-[220px] sm:w-[260px] bg-white border border-[#d4cfc9] rounded-[16px] shadow-lg p-6 flex flex-col items-center relative min-h-[340px]">
            ...
            <div className="w-full pt-3 border-t border-[#eeeae6] text-center">
              <span className="text-[9px] font-mono uppercase tracking-widest text-[#887e8c]">VELLIS · DERMATOLOGY</span>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          <div className="lg:col-span-9 space-y-8">
            <h2 className="font-serif text-4xl sm:text-5xl font-normal leading-tight max-w-3xl">
              How the formula operates on the skin.
            </h2>
      ...
      <section id="directions" className="bg-white text-[#232126] py-20 px-6 sm:px-16">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          ...
          <div className="lg:col-span-9 space-y-8">
            <h2 className="font-serif text-4xl sm:text-5xl font-normal leading-tight max-w-3xl">
              Administration and application guidance.
            </h2>
            ...
            <div className="mt-8 bg-[#efe9f2] border-l-4 border-[#5b2a70] p-5 text-sm leading-relaxed text-[#514b53] max-w-3xl">
              <strong className="text-[#5b2a70] font-semibold">Important:</strong> Use only as directed by a dermatologist or healthcare professional and according to the locally approved product label. Follow physician instructions.
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
            ...
            <div className="lg:col-span-9 space-y-6">
              ...
              <p className="text-base text-[#625d64] max-w-3xl mb-8">
                Review critical safety instructions, dermatological considerations, and contraindications.
              </p>
