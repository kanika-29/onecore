...
import { getNeurologyProductBySlug } from '../data/neurologyProducts';
  ...
  if (parts.length >= 3) {
    ...
    return [
      ...
      },
      {
        ...
        desc: "Supports intended neurological and physiological function."
      }
    ];
  } else {
    return [
      ...
      },
      {
        num: "02",
        title: "Target Pathway",
        desc: "Operates through specific neural receptors and metabolic pathways."
      },
      {
        num: "03",
...
function parsePrecautionsAccordions(precautionsArray) {
  ...
  return precautionsArray.map((text, idx) => {
    ...
    } else if (lower.includes("drowsiness") || lower.includes("sedat") || lower.includes("sleep") || lower.includes("driving")) {
      title = "Sedation, Driving & CNS Precautions";
    } else if (lower.includes("b12") || lower.includes("folate") || lower.includes("anaemia") || lower.includes("haematologic")) {
      title = "B-Vitamin & Haematologic Considerations";
    } else if (lower.includes("pregabalin") || lower.includes("gabapentin") || lower.includes("nortriptyline") || lower.includes("anticholinergic")) {
      title = "Neuropathic Agent & Anticholinergic Warnings";
    } else if (lower.includes("asthma") || lower.includes("bradycardia") || lower.includes("heart block") || lower.includes("hypoglycaemia")) {
      title = "Cardiorespiratory & Metabolic Warnings";
    } else if (lower.includes("steroid") || lower.includes("hyperglycaemia") || lower.includes("infection")) {
      title = "Corticosteroid & Infection Monitoring";
    } else if (lower.includes("serotonin") || lower.includes("clonazepam") || lower.includes("dependence") || lower.includes("withdrawal")) {
      title = "Psychotropic & Dependence Considerations";
    } else if (lower.includes("seizure") || lower.includes("isoflavone") || lower.includes("hormone")) {
      title = "Specialist & Hormone-Sensitive Warnings";
    }
...
export default function NeurologyProductDetail() {
  ...
  useEffect(() => {
    if (product) {
      document.title = `${product.name} — Neurix | Onecore Pharma`;
    } else {
    ...
    return (
      <div className="w-full bg-[#f7f5f1] text-[#232126] min-h-screen">
        <div className="border-b border-[#d9d4cf] py-3 px-6 text-xs text-[#777078]">
          <Link to="/areas-of-care/neurix" className="hover:text-[#232126]">Areas of Care › Neurology</Link>
        </div>
        <div className="max-w-4xl mx-auto px-4 py-20 text-center space-y-6">
          <h1 className="font-serif text-4xl text-[#232126]">Product Not Found</h1>
          <p className="text-base text-[#625d64]">
            The requested Neurology product "<span className="font-mono">{productSlug}</span>" could not be found.
          </p>
          <div className="pt-4">
            <Link
              to="/areas-of-care/neurix"
    ...
    <div className="w-full bg-white text-[#232126] font-sans antialiased">
      ...
      <div className="w-full border-b border-[#d9d4cf] bg-white py-3 px-6 sm:px-12 flex flex-col sm:flex-row justify-between items-start sm:items-center text-xs text-[#777078] gap-2">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <span>›</span>
          <Link to="/areas-of-care/neurix" className="hover:text-[#232126] transition-colors">Neurology</Link>
          <span>›</span>
        ...
        <div className="lg:col-span-5 bg-gradient-to-br from-[#e9e5df] to-[#f1eee9] p-8 sm:p-14 lg:p-16 flex flex-col items-center justify-center relative min-h-[420px]">
          <div className="w-[220px] sm:w-[260px] bg-white border border-[#d4cfc9] rounded-[16px] shadow-lg p-6 flex flex-col items-center relative min-h-[340px]">
            ...
            <div className="w-full pt-3 border-t border-[#eeeae6] text-center">
              <span className="text-[9px] font-mono uppercase tracking-widest text-[#887e8c]">NEURIX · NEUROLOGY</span>
            </div>
          </div>
          <div className="absolute bottom-4 left-6 bg-white/90 backdrop-blur-xs px-3 py-1.5 rounded-xs text-[11px] text-[#736d74] border border-[#e5e1dc]">
            Neurological Healthcare Formulation
          </div>
        </div>
